...
from albert_code.core.config import Backend, VibeConfig
from albert_code.core.llm.format import ResolvedToolCall
from albert_code.core.utils import get_user_agent

if TYPE_CHECKING:
    from albert_code.core.agent_loop import ToolDecision
...
class TelemetryClient:
    ...
    def send_tool_call_finished(
        ...
        *,
        ...
        self.send_telemetry_event("albert_code.tool_call_finished", payload)

    def send_user_copied_text(self, text: str) -> None:
        payload = {"text_length": len(text)}
        self.send_telemetry_event("albert_code.user_copied_text", payload)

    def send_user_cancelled_action(self, action: str) -> None:
        payload = {"action": action}
        self.send_telemetry_event("albert_code.user_cancelled_action", payload)

    def send_auto_compact_triggered(self) -> None:
        payload = {}
        self.send_telemetry_event("albert_code.auto_compact_triggered", payload)
    ...
    def send_slash_command_used(
        ...
        self.send_telemetry_event("albert_code.slash_command_used", payload)
    ...
    def send_new_session(
        ...
        self.send_telemetry_event("albert_code.new_session", payload)

    def send_onboarding_api_key_added(self) -> None:
        self.send_telemetry_event(
            "albert_code.onboarding_api_key_added", {"version": __version__}
        )
