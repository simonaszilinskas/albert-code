...
from albert_code import ALBERT_CODE_ROOT, __version__
from albert_code.acp.acp_logger import acp_message_observer
from albert_code.acp.tools.base import BaseAcpTool
from albert_code.acp.tools.session_update import (
    tool_call_session_update,
    tool_result_session_update,
)
from albert_code.acp.utils import (
    TOOL_OPTIONS,
    ToolOption,
...
from albert_code.core.agents.models import CHAT as CHAT_AGENT, BuiltinAgentName
...
class VibeAcpAgentLoop(AcpAgent):
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        #  - dev mode: `uv run albert-acp`, ran from the project root
        #  - uv tool install: `albert-acp`, similar to dev mode, but uv takes care of path resolution
        #  - bundled binary: `./albert-acp` from binary location
        ...
        # So sys.executable is already /path/to/binary/albert-acp.
        ...
        auth_methods = (
            [
                AuthMethod(
                    ...
                    description="Register your API Key inside Albert Code",
                    field_meta={
                        "terminal-auth": {
                            "command": command,
                            "args": args,
                            "label": "Albert Code Setup",
                        }
                    },
                )
            ]
    ...
    def _get_acp_tool_overrides(self) -> list[Path]:
        ...
        return [
            ALBERT_CODE_ROOT / "acp" / "tools" / "builtins" / f"{override}.py"
        ...
        try:
            if not args:
                ...
                if value is not None:
                    ...
                    message = f"Set `{key}={value}` in ~/.albert-code/.env\n\nPlease start a new chat for changes to take effect."
                else:
                    unset_proxy_var(key)
                    message = f"Removed `{key}` from ~/.albert-code/.env\n\nPlease start a new chat for changes to take effect."
